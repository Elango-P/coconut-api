...
module.exports = {
  async up (queryInterface, Sequelize) {
    try {
      ...
      // Defining the table
      const tableDefinition = await queryInterface.describeTable("location_user_allocation");

      // Condition for removing the portal_id column if it's exist in the table
      if (tableDefinition && tableDefinition["company_id"]) {
        await queryInterface.changeColumn('location_user_allocation', 'company_id',
        {
