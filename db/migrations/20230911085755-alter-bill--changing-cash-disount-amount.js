  ...
  async up (queryInterface, Sequelize) {
    try {
      ...
      if (userTableExists) {
        ...
        if (userTableDefinition && userTableDefinition["cash_discount_amount"]) {
          await queryInterface.changeColumn("bill", "cash_discount_amount", { type: 'DECIMAL USING CAST("cash_discount_amount" as DECIMAL)' });
        };
      };
    } catch (err) {
      console.log(err);
  ...
  async down (queryInterface, Sequelize) {
    try {
      ...
      if (userTableExists) {
        ...
        if (userTableDefinition && userTableDefinition["cash_dicount_amount"]) {
          await queryInterface.changeColumn("bill", "cash_discount_amount", { type: 'INTEGER USING CAST("cash_discount_amount" as INTEGER)' });
        };
      };
    } catch (err) {
      console.log(err);
