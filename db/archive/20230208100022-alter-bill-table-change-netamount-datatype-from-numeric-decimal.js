  ...
  async up (queryInterface, Sequelize) {
    try {
      ...
      if (userTableExists) {
        ...
        if (userTableDefinition && userTableDefinition["net_amount"]) {
          await queryInterface.changeColumn("bill", "net_amount", { type: 'DECIMAL USING CAST("net_amount" as DECIMAL)' });
        };
      };
    } catch (err) {
      console.log(err);
  ...
  async down (queryInterface, Sequelize) {
    try {
      ...
      if (userTableExists) {
        ...
        if (userTableDefinition && userTableDefinition["net_amount"]) {
          await queryInterface.changeColumn("net_amount", "bill", { type: 'NUMERIC USING CAST("net_amount" as NUMERIC)' });
        };
      };
    } catch (err) {
      console.log(err);
