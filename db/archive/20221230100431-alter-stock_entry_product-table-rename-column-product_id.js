...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    try {
      // Console log
      console.log("Altering stock_entry_product table - Renaming column name from product_id to store_product_id");
  ...
  down: async (queryInterface, Sequelize) => {
    try {
      ...
      if (tableDefinition && tableDefinition["store_product_id"] && !tableDefinition["product_id"]) {
        return queryInterface.renameColumn("stock_entry_product", "store_product_id", "product_id");
      };
    } catch (err) {
      console.log(err);
    };
