...
module.exports = async (req, res) => {
  ...
  let params = { companyId: company_id, timeZone : Request.getTimeZone(req),id: id, currentDate: currentDate ,schedularData,roleId:roleId};
  ...
  res.on("finish", async () => {
    ...
    try {
      ...
      if (toMail !== null && toMail.length > 0) {
        ...
        await summaryReportEmailService.sendMail(params, fromMail, toMail, (err) => {
          if (err) {
            console.log(err);
          }
        });
