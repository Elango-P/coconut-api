...
const PurchasePendingReportService = require("../../../services/purchasePendingService");
  ...
  res.on("finish", async () => {
    ...
    try {
      ...
      if (toMail !== null && toMail.length > 0) {
        let PurchaseList = await PurchasePendingReportService.list(Request.GetCompanyId(req));
      ...
      for (let i = 0; i < toMail.length; i++) {
          let sentData = {
              toEmail: toMail[i],
              fromEmail: fromMail,
              subject: "Purchase : Pending Report",
              template: "purchasePendingReport",
